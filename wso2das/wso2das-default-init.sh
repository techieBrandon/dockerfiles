...
if [[ $PLATFORM == "mesos" ]]; then
  ...
  sed -i "/<eventSync>/,/<\/eventSync>/ s|<port>[0-9a-z.]\{1,\}</port>|<port>${PORT18}</port>|g" ${EVENT_PROCESSOR_XML_FILE_PATH} \
  && echo "Replaced eventSync/port with ${PORT18}"
  ...
  sed -i "/<management>/,/<\/management>/ s|<port>[0-9a-z.]\{1,\}</port>|<port>${PORT19}</port>|g" ${EVENT_PROCESSOR_XML_FILE_PATH} \
  && echo "Replaced management/port with ${PORT19}"
  ...
  sed -i "/<presentation>/,/<\/presentation>/ s|<port>[0-9a-z.]\{1,\}</port>|<port>${PORT16}</port>|g" ${EVENT_PROCESSOR_XML_FILE_PATH} \
  && echo "Replaced presentation/port with ${PORT16}"
  ...
  sed -i "s/spark\.driver\.port.*/spark.driver.port ${PORT9}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.driver.port with ${PORT9}"

  sed -i "s/spark\.executor\.port.*/spark.executor.port ${PORT10}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.executor.port with ${PORT10}"
  ...
  sed -i "s/spark\.master\.port.*/spark.master.port ${PORT13}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.master.port with ${PORT13}"
  ...
  sed -i "s/spark\.master\.webui\.port.*/spark.master.webui.port ${PORT15}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.master.webui.port with ${PORT15}"

  sed -i "s/spark\.worker\.port.*/spark.worker.port ${PORT16}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.worker.port with ${PORT16}"

  sed -i "s/spark\.worker\.webui\.port.*/spark.worker.webui.port ${PORT17}/g" ${SPARK_DEFAULTS_CONF_FILE_PATH} \
  && echo "Replaced spark.worker.webui.port with ${PORT17}"
else
  # replace eventSync/hostName
